    ...
    if match := re.match(r"rm_chat\((.+?)\)", query.data):
        ...
        if is_kuki := sql.rem_kuki(chat.id):
            ...
            update.effective_message.edit_text(
                f"Hey Darling Kawaii Chatbot disable by {mention_html(user.id, user.first_name)}.",
...
def kukiadd(update: Update, context: CallbackContext) -> str:
    ...
    if match := re.match(r"add_chat\((.+?)\)", query.data):
        ...
        if is_kuki := sql.set_kuki(chat.id):
            ...
            update.effective_message.edit_text(
                f"Hey Darling Kawaii Chatbot enable by {mention_html(user.id, user.first_name)}.",
                parse_mode=ParseMode.HTML,
            )
...
def list_all_chats(update: Update, context: CallbackContext):
    ...
    text = "<b>Kawaii Controller Enabled Chats</b>\n"
    for chat in chats:
        try:
